...
@showprogress Threads.@threads for data_file in training_data_files
...
for r in eachindex(rrange)
    ...
    RES["AC"][:opinf][:,r]         = ac_opinf[:,r] ./ num_of_training
    RES["AC"][:tropinf][:,r]       = ac_tropinf[:,r] ./ num_of_training
    RES["AC"][:stream_rls][:,r]    = ac_rls[:,r] ./ num_of_training
    RES["AC"][:stream_iqrrls][:,r] = ac_iqrrls[:,r] ./ num_of_training
    RES["AC"][:stream_qrrls][:,r]  = ac_qrrls[:,r] ./ num_of_training
end
...
#================================================#
RES = load(joinpath(FILEPATH, "data/training_statistics.jld2"))
...
le_qrrls   = zeros(max_num_of_LE, length(rrange), num_of_training)
...
ky_qrrls   = zeros(length(rrange), num_of_training)
