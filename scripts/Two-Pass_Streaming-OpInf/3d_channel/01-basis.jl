...
## baker
tmp = @elapsed baker = iSVD(x1=ds[1], algo=:baker, max_rank=rmax)
push!(time_baker, tmp)
@showprogress for i in 2:n 
    tmp = @elapsed increment!(baker, ds[i])
    push!(time_baker, tmp)
end
...
for i in [100]
    mult = 1
    tot_norm = 0.0
    # @showprogress @inbounds @simd for j in 1:(n ÷ mult)
    for j in 1:n
        # X = ds[mult*(j-1)+1:mult*j]
        X = ds[j]
