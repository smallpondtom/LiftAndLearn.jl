...
num_of_streams = 5000  # -1 for the derivative and -1 for the removed last state
rspan = 1:rmax
tmp_res = (
...
for (file_idx, data_file) in enumerate(training_data_files)
    jldopen(data_file, "r") do data
        ...
        X = data["X"]
        U = data["U"]
        U = reshape(U, size(U,1), :)  # Ensure that U is a matrix
        Xdot = data["Xdot"]
        Xref = data["Xref"]
        Uref = data["Uref"]
        ...
        for (k, rloop) in enumerate(rspan)
            ...
            op_inf = LnL.opinf(X, iVr, options; U=U, Xdot=Xdot)

            # Tikhonov Regularized OpInf
            options.with_reg = true
            options.λ = LnL.TikhonovParameter(A=Γ, B=Γ, A2=Γ)
            op_trinf = LnL.opinf(X, iVr, options; U=U, Xdot=Xdot)
            ...
            rls_stream  = LnL.TwoPassStreamingOpInf(
                options=options, n=rloop, m=1, algorithm=:RLS
            ) 
