...
Xtrain = reshape(X, burgers.spatial_dim, :)
Utrain = reshape(U, :, 1)
Xfull_flat = reshape(Xfull, burgers.spatial_dim, :)
...
## Get the right indices and finite difference matrices 
Es = AbstractMatrix{Float64}[]
for i in axes(Xfull, 3)
    Xi = Xfull[:,:,i]
    E, _ = LnL.finite_diff_matrix(
...
for i in axes(X, 3)
    Xi = X[:,:,i]
    _, Δidx = LnL.finite_diff_matrix(
        options.data.deriv_type, size(Xi,2), options.data.Δt
    )
    push!(Δidxs, Δidx .+ shift)
    shift += size(Xi, 2)
end
Δidx = reduce(vcat, Δidxs)
...
op_infer = LnL.opinf(
    ...
    options; 
    U=Utrain[Δidx],
    Xhatdot=((Vrmax' * Xfull_flat) * E)[:,1:options.data.DS:end],
)
Ainf = op_infer.A
Binf = op_infer.B 
Finf = op_infer.A2u
...
# )
# @showprogress for xi in eachcol(Xtrain[:,2:end])
#     LnL.stream!(stream, xi, tol=1e-8)
# end

# ##
# op_stream = LnL.compute_stream_operators(stream, E, Δidx; U=Utrain)
