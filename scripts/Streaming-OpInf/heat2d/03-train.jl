...
#==================================#
## Generate iQRRLS-Streaming-OpInf 
#==================================#
# Placeholders
r = basis_data["r"]
num_of_streams = heat2d.time_dim
...
## Save the streaming results
filename = joinpath(FILEPATH, "data/streaming", "iqrrls_stream_results.jld2")
save(
    filename,
    "state_stream_res", state_stream_res, "output_stream_res", output_stream_res
)
...
#=================================#
## Generate QRRLS-Streaming-OpInf 
#=================================#
# Placeholders
r = basis_data["r"]
num_of_streams = heat2d.time_dim
...
for (i, data_file) in enumerate(training_data_files)
    jldopen(data_file, "r") do data
        ...
        # γs = 1e-9
        # γo = 1e-9
        state_stream, output_stream = LnL.StreamingOpInf(options=options, n=r, m=4, l=1, algorithm=:QRRLS, γs=γs, γo=γo)
...
## Save the streaming results
filename = joinpath(FILEPATH, "data/streaming", "qrrls_stream_results.jld2")
save(
    filename,
    "state_stream_res", state_stream_res, "output_stream_res", output_stream_res
)