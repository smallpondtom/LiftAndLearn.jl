...
function StreamingOpInf(options::AbstractOption, n::Int, m::Int=0, l::Int=0; 
                        variable_regularize::Bool=false,
    ...
    O_k = zeros(d,n)
    P_k = iszero(γs_k) ? zeros(d,d) : 1.0I(d) / γs_k
    K_k = []
    C_k = zeros(n,l)
    Py_k = iszero(γo_k) ? zeros(n,n) : 1.0I(n) / γo_k
    Ky_k = []
    ...
    return StreamingOpInf(
        ...
        stream!, stream_output!, unpack_operators
    )
end
